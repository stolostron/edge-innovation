...
demo_cluster_config=${REPO_DIR}/sensor-aggregator/kind/config.yaml

spoke_cluster_name="edge"

addon_namespace="open-cluster-management-agent-addon"
addon_deploy=${REPO_DIR}/sensor-aggregator/deploy

opcua_server=${REPO_DIR}/sensor-aggregator/opcuaserver
...
kind load docker-image quay.io/skeeey/sensor-aggregator --name=${demo_cluster}
...
sh -c "$(cat ${REPO_DIR}/sensor-aggregator/join.sh) ${spoke_cluster_name} --feature-gates=AddonManagement=true --force-internal-endpoint-lookup --context ${demo_cluster_context}"
sleep 30
